...
export const ResearchesListPage: IFC = observer(() => {
  ...
  const createNewResearch = useMutation({
    mutationFn: (newResearchData: TCreateResearchData) => researchApi.createResearch(newResearchData),
    onSuccess: (responseData) => researchesStore.createNewResearch(responseData.data.payload!),
  });

  return (
    <div className={s.researchesContainer}>
      {!researchesList.length && <ResearchesEmpty onCreateResearch={createNewResearch.mutate} />}
    </div>
  );
}, 'ResearchesListPage');
